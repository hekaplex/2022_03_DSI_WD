...
total_ceballos = sum(
    [1 for response in survey_responses if response == 'Ceballos'])
...
possible_surveys = np.random.binomial(
    survey_length, .54, size=10000)/survey_length
...
large_survey = np.random.binomial(
    large_survey_length, .54, size=10000) / large_survey_length

incorrect_precictions = len(large_survey[large_survey < .5])
