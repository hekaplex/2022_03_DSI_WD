...
--subquery
--CREATE PROCEDURE sample_report @state varchar(2) = 'CA'
--AS
SELECT 
...
WHERE
		...
		(
			...
			FROM
				Vendors
			WHERE
				VendorState = 'CA' --@state
		)

--		EXEC sample_report 'MA'
--relational algebra
		...
		AS
			(
				...
				SELECT
					...
					from employees
						JOIN EmployeesCTE
						ON Employees.ManagerID = EmployeesCTE
			)